...
TallyVotes::Tour TallyVotes::vote(unsigned int id, Tour ballot) {
	unsigned int myTicket = ++ticket;//assign each voter is a ticket
	//check if voter is a barger
	while(true){
		if(myTicket <= group * signalling) break;//wait until ticket belongs to group
	...
	if(curMember < group){
		...
		while(true){
			wait();
			if(myTicket <= group * signalling) break; //check if ticket is being signalled
		}
	...
	if(curMember == 0){
		signalling += 1;//signal the next group
		signalAll();//wake up all waiting and check if they are called upon
	}
...
//wait routine to accept bargers
