...
_Coroutine Player {
  ...
  static unsigned int nPlayer; 
	...
	//for death deck remove player pointer from list
	void removeRight(Player * rp){ Player::rp = rp; }//change the rp to the new rp
	void removeLeft(Player * lp) { Player::lp = lp; }//change the lp to the new lp
  public:
...
class Printer {
  ...
  public:
    Printer( const unsigned int NoOfPlayers, const unsigned int NoOfCards ) : 
		NoOfPlayers(NoOfPlayers), NoOfCards(NoOfCards), cRemain(NoOfCards), cnPlayers(NoOfPlayers){
		took = new unsigned int[NoOfPlayers];
		remaining = new unsigned int[NoOfPlayers];
		//print the initial outputs(Players: N , Cards : M)
